...
const (
	...
	// DelegateSourceIPVerification is the namespace-level permission gate; must be
	// set by cluster admins before pod-level source IP verification overrides take effect.
	DelegateSourceIPVerification = ConfigPrefix + "/delegate-source-ip-verification"

	// DisableSourceIPVerification disables source IP verification for the annotated pod.
	// Only effective when the namespace has DelegateSourceIPVerification=true.
