...
// TestSyncIPCache_NilIPCache verifies the announcer is a no-op for ipcache
// updates when no ipcache is wired up (legacy callers / unit tests that don't
// inject one). This guards against the nil-deref regression that bit the
// initial implementation.
func TestSyncIPCache_NilIPCache(t *testing.T) {
	fix := newFixture(t)
	require.Nil(t, fix.announcer.ipc, "fixture should not wire up an ipcache")
