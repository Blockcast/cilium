 ...
 * is_valid_lxc_src_ip checks the strict (saddr == endpoint_ip) condition;
 * if that fails, callers also probe the L2 responder map. Pods running the
 * kernel AMT relay (drivers/net/amt.c) and similar modules legitimately
 * source packets with a Service ExternalIP/VIP (RFC 7450 §5.1.2). Mirror
 * b5b3b4b908 (sock4_skip_xlate): trust addresses present in the L2
 * responder map for direct_routing_dev_ifindex on this node.
 *
 * Kept here (not in lib/lxc.h) so lib/l2_responder.h stays out of lxc.h's
 * include chain — l2_responder.h carries the full handle_l2_announcement
 * body, which references arp/icmp6/runtime-config symbols only available
 * in BPF programs that pull arp.h + icmp6.h + the per-endpoint config.
 * bpf_lxc.c already pulls those; tests and other lxc.h consumers don't.
 *
 ...
 * use hostNetwork (cleaner). See the chart-side staging/prod values and
 * the L2 policy in onprem-k8s cilium-l2-config.yaml.
 */
