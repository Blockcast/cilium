/* SPDX-License-Identifier: (GPL-2.0-only OR BSD-2-Clause) */
/* Copyright Authors of Cilium */

/* Map declarations for the L2 announcement responder. This header is
 * include-safe from any BPF program: it does not pull in agent-config
 * symbols (interface_mac, interface_ifindex, ...) that only bpf_lxc.c
 * sees. Programs that implement the responder itself include
 * "lib/l2_responder.h", which adds the handle_l2_announcement helper.
 */

#pragma once

#include <linux/bpf.h>
#include <bpf/section.h>
#include <bpf/loader.h>

struct l2_responder_v4_key {
	union v4addr ip4;
	__u32 ifindex;
};

struct l2_responder_stats {
	__u64 responses_sent;
};

struct {
	__uint(type, BPF_MAP_TYPE_HASH);
	__type(key, struct l2_responder_v4_key);
	__type(value, struct l2_responder_stats);
	__uint(pinning, LIBBPF_PIN_BY_NAME);
	__uint(max_entries, L2_RESPONDER_MAP4_SIZE);
	__uint(map_flags, BPF_F_NO_PREALLOC);
} cilium_l2_responder_v4 __section_maps_btf;

struct l2_responder_v6_key {
	union v6addr ip6;
	__u32 ifindex;
	__u32 pad;
};

struct {
	__uint(type, BPF_MAP_TYPE_HASH);
	__type(key, struct l2_responder_v6_key);
	__type(value, struct l2_responder_stats);
	__uint(pinning, LIBBPF_PIN_BY_NAME);
	__uint(max_entries, L2_RESPONDER_MAP6_SIZE);
	__uint(map_flags, BPF_F_NO_PREALLOC);
} cilium_l2_responder_v6 __section_maps_btf;
