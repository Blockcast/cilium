...
/*
 * Lightweight L2-responder map + key definitions, split out of
 * l2_responder.h so translation units that only need to look up the
 * responder maps (e.g. bpf_sock.c sockLB, lxc.h) can include them
 * WITHOUT pulling in handle_l2_announcement(), which depends on
 * arp.h / icmp6.h helpers only present in the bpf_host.c compile unit.
 */
